...
$writeOptions = parseOptions([
    'clone',
    '--source-db=fixture_source',
    '--target-db=fixture_target',
    '--manifest-dir=runtime/isolated-formal-validation-fixture_a',
    '--database-host=127.0.0.1',
    '--run-label=formal-fixture',
    '--run-date=20300102',
    '--expected-table-count=8',
    '--expected-foreign-key-count=2',
    '--confirm-create-target=fixture_target',
]);
if (count($writeOptions) !== 9) {
    throw new RuntimeException('clone option parser lost an explicit write option');
}
$preflightOptions = parseOptions([
    'clone',
    '--source-db=fixture_source',
    '--target-db=fixture_target',
    '--manifest-dir=runtime/isolated-formal-validation-fixture_b',
...
sourceDatabase($writeOptions['source-db']);
targetDatabase($writeOptions['target-db']);
...
if (!str_starts_with($targetDdl, 'CREATE TABLE `fixture_target`.`act_ru_task`')
...
$foreignKeyRows = [[
    ...
    'referenced_table_schema' => 'source_fixture',
    'referenced_table_name' => 'parent',
    'referenced_column_name' => 'id',
    'unique_constraint_schema' => 'source_fixture',
...
$foreignKeys = normalizeForeignKeyDefinitions($foreignKeyRows, 'source_fixture');
$reversedForeignKeys = normalizeForeignKeyDefinitions(array_reverse($foreignKeyRows), 'source_fixture');
...
clone_smoke_expect_failure(static fn () => parseOptions([
    ...
    '--expected-table-count=8',
    '--expected-foreign-key-count=2',
    '--confirm-create-target=wrong_target',
]));
clone_smoke_expect_failure(static fn () => parseOptions([
    'clone',
    '--source-db=fixture_source',
    '--target-db=fixture_source',
    '--manifest-dir=runtime/x',
    '--database-host=127.0.0.1',
    '--run-label=formal-fixture',
    '--run-date=20300102',
...
clone_smoke_expect_failure(static fn () => sourceDatabase('mysql'));
clone_smoke_expect_failure(static fn () => sourceDatabase('fixture_source;DROP'));
clone_smoke_expect_failure(static fn () => targetDatabase('performance_schema'));
clone_smoke_expect_failure(static fn () => assertLoopbackHost('192.0.2.1'));
clone_smoke_expect_failure(static fn () => assertLoopbackHost(
    'remote-db',
    static fn (string $host): array => $host === 'remote-db' ? ['192.0.2.1'] : []
));
clone_smoke_expect_failure(static fn () => assertLoopbackHost(
    'mixed-db',
    static fn (string $host): array => $host === 'mixed-db' ? ['127.0.0.1', '192.0.2.1'] : []
));
...
clone_smoke_expect_failure(static fn () => targetCreateTableDdl(
    ['Create Table' => 'CREATE TABLE `act_ru_task` (`id` int, FOREIGN KEY (`id`) REFERENCES `source_fixture`.`parent` (`id`))'],
    'act_ru_task',
    'fixture_target'
));
clone_smoke_expect_failure(static fn () => normalizeForeignKeyDefinitions(
    [array_merge($foreignKeyRows[0], ['referenced_table_schema' => 'other_fixture'])],
    'source_fixture'
));
