...
$prefix = 'CODEX_MOBILE_BUTTON_' . date('YmdHis') . random_int(1000, 9999);
$createdIds = [];
$relationId = '';
$createdMenu = false;

try {
    $parent = think\facade\Db::name('mobile_resource')
        ->where('CATEGORY', 'MENU')
        ->where(function ($query): void {
            $query->whereNull('DELETE_FLAG')->whereOr('DELETE_FLAG', '=', 'NOT_DELETE');
        })
    ...
    $buttonId = (string)($created['id'] ?? '');
    if ($buttonId === '') {
        throw new RuntimeException('mobile button add did not return id');
    }
    $createdIds[] = $buttonId;
    ...
    if (!$duplicateFailed) {
        throw new RuntimeException('duplicate mobile button code should fail');
    }
    ...
    if (($edited['title'] ?? '') !== $prefix . '_EDITED' || ($edited['sortCode'] ?? 0) !== 9997) {
        throw new RuntimeException('mobile button edit did not update title/sortCode');
    }

    $relationId = 'cmobrel-' . random_int(100000, 999999);
    think\facade\Db::name('sys_relation')->insert([
        'ID' => $relationId,
        'OBJECT_ID' => 'codex-role',
        'TARGET_ID' => $parentId,
        'CATEGORY' => 'SYS_ROLE_HAS_MOBILE_MENU',
        'EXT_JSON' => json_encode(['buttonInfo' => [$buttonId, 'keep-mobile-button']], JSON_UNESCAPED_SLASHES),
    ]);

    $deleted = $service->buttonDelete([['id' => $buttonId], ['id' => 'missing-mobile-btn']], $payload);
    if (($deleted['count'] ?? 0) !== 1) {
        throw new RuntimeException('mobile button delete count mismatch');
    }

    $deleteFlag = (string)think\facade\Db::name('mobile_resource')->where('ID', $buttonId)->value('DELETE_FLAG');
    if ($deleteFlag !== 'DELETED') {
        throw new RuntimeException('mobile button was not logically deleted');
    }

    $extJson = json_decode((string)think\facade\Db::name('sys_relation')->where('ID', $relationId)->value('EXT_JSON'), true);
    if (($extJson['buttonInfo'] ?? []) !== ['keep-mobile-button']) {
        throw new RuntimeException('role mobile menu buttonInfo cleanup failed');
    }

    echo "MobileResourceService button write checks passed\n";
} finally {
    if ($relationId !== '') {
        think\facade\Db::name('sys_relation')->where('ID', $relationId)->delete();
    }
    ...
    if ($createdMenu) {
        think\facade\Db::name('mobile_resource')->where('ID', $parentId)->delete();
    }
}
'@

    $probePath = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-mobile-resource-button-smoke-$([guid]::NewGuid()).php")
    try {
        Set-Content -LiteralPath $probePath -Value $probe -Encoding UTF8
        $output = & php $probePath 2>&1
        if ($LASTEXITCODE -ne 0) {
            throw "php MobileResourceService button write probe failed: $output"
        }
    } finally {
        if (Test-Path -LiteralPath $probePath) {
            Remove-Item -LiteralPath $probePath -Force
