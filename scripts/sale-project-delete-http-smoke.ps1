...
function Invoke-RawPostJson {
    ...
    $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-sale-project-delete-{0}.json" -f ([Guid]::NewGuid().ToString('N')))
    try {
...
`$auth['device'] = 'CODEX_SALE_PROJECT_DELETE_HTTP_SMOKE';
echo json_encode([
...
$prefix = 'CODEX_DELETE_' + (Get-Date -Format 'MMddHHmmss') + '_' + (Get-Random -Minimum 1000 -Maximum 9999)
$projectIdOne = [string]([Int64]604400000000000000 + [Int64](Get-Random -Minimum 100000 -Maximum 999999))
$projectIdTwo = [string]([Int64]604401000000000000 + [Int64](Get-Random -Minimum 100000 -Maximum 999999))
$invalidProjectId = [string]([Int64]604402000000000000 + [Int64](Get-Random -Minimum 100000 -Maximum 999999))
$missingId = [string]([Int64]604499000000000000 + [Int64](Get-Random -Minimum 100000 -Maximum 999999))
$invoiceId = [string]([Int64]704400000000000000 + [Int64](Get-Random -Minimum 100000 -Maximum 999999))
...
"@
    ...
    $noToken = Invoke-RawPostJson -Url "$baseUrl/biz/saleproject/delete" -Data @(@{ id = $projectIdOne })
    Assert-Code -Json $noToken -Expected 401 -Name 'sale project delete without token'

    $missingList = Invoke-RawPostJson -Url "$baseUrl/biz/saleproject/delete" -Token $token -Data @{ items = @() }
    Assert-Code -Json $missingList -Expected 400 -Name 'sale project delete missing list'

    $missingProjectRow = Invoke-RawPostJson -Url "$baseUrl/biz/saleproject/delete" -Token $token -Data @(@{ id = $missingId })
    Assert-Code -Json $missingProjectRow -Expected 404 -Name 'sale project delete missing row'

    $invalidState = Invoke-RawPostJson -Url "$baseUrl/biz/saleproject/delete" -Token $token -Data @(@{ id = $invalidProjectId })
    Assert-Code -Json $invalidState -Expected 400 -Name 'sale project delete invalid state'
...
"@
    if ($invalidAfter.PROJECT_STATE -ne 'WAIT_DELIVER' -or [int]$invalidAfter.VERSION -ne 0 -or $invalidAfter.DELETE_FLAG -ne 'NOT_DELETE') {
        throw 'sale project delete invalid-state request changed the project'
    }

    $valid = Invoke-RawPostJson -Url "$baseUrl/biz/saleproject/delete" -Token $token -Data @(
        @{ id = $projectIdOne },
        @{ id = $projectIdTwo }
    )
    Assert-Code -Json $valid -Expected 200 -Name 'sale project delete valid'
...
"@
    foreach ($project in $after.projects) {
        if ($project.PROJECT_STATE -ne 'DISCARD' -or $project.DELETE_FLAG -ne 'DELETED') {
            throw 'sale project delete did not soft-delete the project as expected'
        }
        if ([int]$project.VERSION -ne 1 -or [string]$project.UPDATE_USER -ne $userId -or [string]$project.UPDATE_TIME -eq '') {
            throw 'sale project delete did not refresh audit/version fields'
        }
    }
    if ($after.invoice.DELETE_FLAG -ne 'NOT_DELETE') {
        throw 'sale project delete unexpectedly changed invoicing rows'
    }

    Write-Host 'sale project delete HTTP smoke passed'
} finally {
    Invoke-Php -Code $cleanupCode | Out-Null
}
