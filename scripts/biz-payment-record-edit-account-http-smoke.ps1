...
function Invoke-RawPostJson {
    ...
    $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-payment-record-edit-account-{0}.json" -f ([Guid]::NewGuid().ToString('N')))
    try {
...
$prefix = 'codex-payacct-' + ([Guid]::NewGuid().ToString('N').Substring(0, 8))
$payerTime = '2026-04-05 06:07:08'
...
`$auth['device'] = 'CODEX_PAYMENT_RECORD_EDIT_ACCOUNT_HTTP_SMOKE';
echo json_encode([
...
$targetOrgId = [string]$setup.targetOrgId
if ($token.Trim() -eq '' -or $tenantId.Trim() -eq '') {
    throw 'failed to set up payment record edit-account smoke data'
}

$baseUrl = $BackendBaseUrl.TrimEnd('/')

try {
    $noToken = Invoke-RawPostJson -Url "$baseUrl/biz/bizpaymentrecord/edit/account" -Data @{
        id = $recordId
        currentTargetId = $currentAccountId
        targetId = $targetAccountId
    }
    Assert-Code -Json $noToken -Expected 401 -Name 'payment record edit-account without token'

    $missing = Invoke-RawPostJson -Url "$baseUrl/biz/bizpaymentrecord/edit/account" -Token $token -Data @{
        id = $recordId
        currentTargetId = $currentAccountId
    }
    Assert-Code -Json $missing -Expected 400 -Name 'payment record edit-account missing targetId'

    $same = Invoke-RawPostJson -Url "$baseUrl/biz/bizpaymentrecord/edit/account" -Token $token -Data @{
        id = $recordId
        currentTargetId = $currentAccountId
        targetId = $currentAccountId
    }
    Assert-Code -Json $same -Expected 400 -Name 'payment record edit-account same account'

    $mismatch = Invoke-RawPostJson -Url "$baseUrl/biz/bizpaymentrecord/edit/account" -Token $token -Data @{
        id = $recordId
        currentTargetId = $targetAccountId
        targetId = $currentAccountId
    }
    Assert-Code -Json $mismatch -Expected 400 -Name 'payment record edit-account current mismatch'

    $missingStatement = Invoke-RawPostJson -Url "$baseUrl/biz/bizpaymentrecord/edit/account" -Token $token -Data @{
        id = $rollbackRecordId
        currentTargetId = $currentAccountId
        targetId = $targetAccountId
    }
    Assert-Code -Json $missingStatement -Expected 404 -Name 'payment record edit-account missing statement rollback'
...
echo json_encode([
    ...
    'paymentTargetId' => (string)(`$payment['TARGET_ID'] ?? ''),
...
"@
    ...
    Assert-Equal -Actual ([string]$before.paymentTargetId) -Expected $currentAccountId -Name 'failed cases payment target preserved'
    Assert-Equal -Actual ([string]$before.rollbackTargetId) -Expected $currentAccountId -Name 'missing statement payment target preserved'
    ...
    $edit = Invoke-RawPostJson -Url "$baseUrl/biz/bizpaymentrecord/edit/account" -Token $token -Data @{
        id = $recordId
        currentTargetId = $currentAccountId
        targetId = $targetAccountId
        amount = 9999.99
    ...
    Assert-Code -Json $edit -Expected 200 -Name 'payment record edit-account'
    Assert-PathEquals -Json $edit -Path 'data.id' -Expected $recordId -Name 'payment record edit-account id'
    Assert-PathEquals -Json $edit -Path 'data.statementId' -Expected $statementId -Name 'payment record edit-account statement id'
    Assert-PathEquals -Json $edit -Path 'data.currentTargetId' -Expected $currentAccountId -Name 'payment record edit-account current target id'
    Assert-PathEquals -Json $edit -Path 'data.targetId' -Expected $targetAccountId -Name 'payment record edit-account target id'
    Assert-PathEquals -Json $edit -Path 'data.amount' -Expected '12.34' -Name 'payment record edit-account stored amount'

    $detail = Invoke-RawGet -Url "$baseUrl/biz/bizpaymentrecord/detail?id=$(Enc $recordId)" -Token $token
    Assert-Code -Json $detail -Expected 200 -Name 'payment record detail after account switch'
    Assert-PathEquals -Json $detail -Path 'data.targetId' -Expected $targetAccountId -Name 'payment record detail targetId'
    Assert-PathEquals -Json $detail -Path 'data.org' -Expected $targetOrgId -Name 'payment record detail org'
...
"@
    ...
    Assert-DecimalEqual -Actual ([string]$state.currentAccount.amount) -Expected ([decimal]'987.66') -Name 'current account amount decreased by stored payment amount'
    Assert-DecimalEqual -Actual ([string]$state.targetAccount.amount) -Expected ([decimal]'512.34') -Name 'target account amount increased by stored payment amount'
    ...
    Assert-Equal -Actual ([string]$state.payment.targetId) -Expected $targetAccountId -Name 'payment targetId switched'
    ...
    Assert-Equal -Actual ([string]$state.payment.payerTime) -Expected $payerTime -Name 'payment payerTime preserved'
    ...
    Assert-Equal -Actual ([string]$state.payment.org) -Expected $targetOrgId -Name 'payment org switched to target account org'
    ...
    Assert-Equal -Actual ([string]$state.rollback.targetId) -Expected $currentAccountId -Name 'rollback payment targetId preserved'
    Assert-Equal -Actual ([string]$state.rollback.serialId) -Expected $missingStatementId -Name 'rollback payment serialId preserved'
    Assert-DecimalEqual -Actual ([string]$state.rollback.amount) -Expected ([decimal]'12.34') -Name 'rollback payment amount preserved'
    ...
    Write-Host 'biz payment record edit-account HTTP smoke passed'
} finally {
    Invoke-Php -Code $cleanupCode | Out-Null
}
