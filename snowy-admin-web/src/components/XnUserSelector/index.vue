	...
	<div style="display: flex" v-if="props.userShow">
		...
		>
			<span class="user-delete">
				<CloseCircleFilled
					:class="index === deleteShow && closeShow ? 'show-delete-icon' : ''"
					class="delete-icon"
					@click="deleteUser(user)"
				/>
				<a-avatar :src="user.avatar" />
			</span>
			<span class="user-name" v-if="userNameShow">{{ user.name }}</span>
		</div>
		<a-button
			:disabled="disabled"
			shape="circle"
			@click="openModal"
			v-if="(props.radioModel ? userObj.length !== 1 : true) && addShow"
		>
			<PlusOutlined />
		</a-button>
		...
		if (isEmpty(userObj.value) && !isEmpty(ids)) {
			...
			props.userListByIdListApi(param).then((data) => {
				...
				ids.forEach((id) => {
					...
					if (user) sortedData.push(user)
				})
				userObj.value = sortedData
			})
		}
	}
	watch(
