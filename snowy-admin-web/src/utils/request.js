...
import qs from 'qs'
import { Modal, message, notification,Input  } from 'ant-design-vue'
...
service.interceptors.request.use(
	(config) => {
		...
		if(tenantId&&!config.headers.tenantId){
			config.headers.tenantId =  tool.data.get('tenantId');
		}
		if (token) {
	...
	Modal.confirm({
		...
		content: createVNode(
			Input.Password,
			{
				...
				'onUpdate:visible': (newVisible) => (visible.value = newVisible),
			},
			null
		),
		okText: '确认',
		...
		async	onOk() {
			...
			const api = await import('@/api/auth/safeApi');

			api.default.openSafeByPassword({
...
service.interceptors.response.use(
	(response) => {
		...
		if(response.headers.getContentType() === 'text/event-stream'){
			return  response;
		}
		...
		if(reloadPassword.includes(code)){

				safePassword(data.data);
				return ;
