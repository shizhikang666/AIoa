...
//多层级翻译方法
tool.dictTypeDataByPath = (...dictValue) => {
	let value = dictValue.pop()
	let children = tool.dictListByPath(dictValue)
	const dict = children.find((item) => item.value === value)

	return dict ? dict.label : '无此字典项'
}

tool.dictTypeDataByPathReturnEmpty = (...dictValue) => {
	let value = dictValue.pop()
	let children = tool.dictListByPath(dictValue)
	const dict = children.find((item) => item.value === value)
...
tool.dictTypeList = (dictValue) => {
	...
	const findTree = (treeNodes, values) => {
		if (!treeNodes || values.length === 0) {
			return treeNodes || []
		}

		const [currentValue, ...restValues] = values
		const tree = treeNodes.find((item) => item.dictValue === currentValue)
		...
		// 继续向下查找
		return findTree(tree.children, restValues)
	}

	const dictValues = Array.isArray(dictValue) ? dictValue : [dictValue]
	return findTree(dictTypeTree, dictValues)
}
...
tool.dictListByPath = (...args) => {
	...
	let parentTree = tool.dictDataAll()
	if (!parentTree) {
		return []
	}

	const traverse = (paths, parentTree) => {
		if (paths.length === 0) {
			return []
		}

		let dictValue = paths.shift()
		const tree = parentTree.find((item) => item.dictValue === dictValue)
		...
		if (paths.length > 0) {
			return traverse(paths, tree.children)
		}
	...
	return traverse(paths, parentTree)
}
