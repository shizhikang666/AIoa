<template>
	...
	<update-user-info ref="updateUserInfoRef" />
</template>

<script setup>
	import { message } from 'ant-design-vue'
	import UpdatePassword from './bindForm/updatePassword.vue'
	import UpdateUserInfo from './bindForm/updateUserInfo.vue'
	import { useTemplateRef } from 'vue'

	const updateUserInfoRef = useTemplateRef('updateUserInfoRef')
	const updatePasswordRef = ref()
	// 获取绑定的情况
	const data = [
		{ title: '密码强度', description: '当前密码强度', value: '弱', type: 'password', bindStatus: 0 },
		{ title: '个人资料', description: '资料', value: '去编辑', type: 'userinfo', bindStatus: 0 },
	...
	const bindCommon = (key) => {
		if (key === 'password') {
			...
			updateUserInfoRef.value.onOpen()
		} else {
			message.info('开发中')
		}
	}
