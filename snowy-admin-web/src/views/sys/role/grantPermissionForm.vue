				...
				<template #headerCell="{ column }">
					...
					<template v-if="column.key === 'dataScope'">
						...
						<a-radio-group v-model:value="scopeRadioValue" @change="radioChange" style="padding: 0 10px">
							...
							<a-radio value="SCOPE_COMPANY_CHILD"> 所属公司及以下 </a-radio>
						</a-radio-group>
					</template>
				</template>
				...
				<template #bodyCell="{ column, record }">
					...
					<template v-if="column.dataIndex === 'dataScope'">
						<template v-if="record.dataScope.length > 0">
							...
							<a-button v-if="record.dataScope[5].check" type="link" size="small" @click="handleDefineOrg(record)"
								>选择组织</a-button
							>
						</template>
					</template>
...
<script setup name="grantResourceForm">
	...
	const datascope = (id) => {
		return [
			...
			{
				id: `SCOPE_COMPANY_CHILD_${id}`,
				title: '所属公司及以下',
				value: 'SCOPE_COMPANY_CHILD',
				check: false
			},
	...
	const handleDefineOrg = (recordDataScope) => {
		...
		if (data.check) {
			...
			const checkKeysStr = recordDataScope.dataScope[5].scopeDefineOrgIdList
	...
	// 截取api串中的中文及括号
	const subStrApi = (api) => {
		const apiText = String(api || '').trim()
		const splitIndex = apiText.indexOf('[')
		return splitIndex > -1 ? apiText.substring(0, splitIndex) : apiText
	}
	// 验证并提交数据
	const onSubmit = () => {
