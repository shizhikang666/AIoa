<template>
	...
	>
		<a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical" :label-col="labelCol">
			...
			<a-form-item label="业务系统显示：" name="viewState">
				<a-select v-model:value="formData.viewState"  :options="viewStateOptions" />
			</a-form-item>
			<a-form-item label="业务可添加：" name="editState">
				<a-select  v-model:value="formData.editState"  :options="editStateOptions" />
			</a-form-item>
...
<script setup name="dictForm">
	...
	const  editStateOptions = ref([])
	const  viewStateOptions = ref([])


	// 打开抽屉
	const onOpen = (record, type, parentId) => {
		viewStateOptions.value =  tool.dictListByPath(["DICT_VIEW_STATE"]);
		editStateOptions.value =  tool.dictListByPath(["DICT_EDIT_STATE"]);
