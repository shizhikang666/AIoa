				...
				>
					<template #operator class="table-operator">
						<a-space>
							...
							/>
							<a-button @click="exportBatchUserInfo" v-if="hasPerm('bizUserExportUserInfo')"> 导出信息模板 </a-button>
						</a-space>
					</template>
							...
							<a-dropdown v-if="hasPerm(['bizUserGrantRole', 'bizUserPwdReset', 'bizUserExportUserInfo'])">
								...
								<template #overlay>
									<a-menu>
										...
										<a-menu-item v-if="hasPerm('bizUserExportUserInfo')">
											<a @click="exportUserInfoWord(record)">导出个人档案信息</a>
										</a-menu-item>
									</a-menu>
								</template>
							</a-dropdown>
...
<script setup name="bizUser">
	...
	const exportBatchUserInfo = async (param) => {
		const res = await bizUserApi.userList(param)
		for (let i = 0, len = res.length; i < len; i++) {
			await exportUserWord(res[i])
		}
	}
	...
	const exportUserInfoWord = async (record) => {
		const params = {
			id: record.id
		}
		const detail = await bizUserApi.userDetail(params)
		exportUserWord(detail)
	}
