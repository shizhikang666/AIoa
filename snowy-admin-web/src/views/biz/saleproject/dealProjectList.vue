<template>
					...
					<a-dropdown v-if="hasPerm(['bizSaleProjectDetail', 'bizSaleProjectStartProcess', ''])">
						...
						<template #overlay>
							<a-menu>
								...
								<a-menu-item v-if="hasPerm('bizSaleProjectAmountEdit')">
									<a-anchor-link @click="openEditAmountFormRef(record)">修改项目金额</a-anchor-link>
								</a-menu-item>
	...
	<editAmountForm ref="editAmountFormRef" />
</template>
<script setup name="dealProjectList">
	...
	const editAmountFormRef = useTemplateRef('editAmountFormRef')
	...
	const openEditAmountFormRef = (record) => {
		editAmountFormRef.value.onOpen(record)
	}
