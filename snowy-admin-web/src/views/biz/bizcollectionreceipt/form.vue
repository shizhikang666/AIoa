<template>
	...
	>
		<a-tabs v-model:activeKey="activeKey">
			<a-tab-pane tab="支出信息" key="baseInfo" >
				<a-form   ref="formRef"  :model="formData" :rules="formRules"   layout="vertical">
					<template v-for="(item,index) in formData.formDataList">
						<a-descriptions :column="2" size="small" bordered>
							...
							<a-descriptions-item label="总收款">{{item.loanAmount}}</a-descriptions-item>
							<a-descriptions-item label="已支出">
								{{item.settlementAmount}}
							</a-descriptions-item>
							<a-descriptions-item label="剩余未支出">
								{{item.maxAmount}}
							</a-descriptions-item>
			...
			<a-tab-pane tab="还款账户信息" key="accountInfo">
				<a-form   ref="accountFormRef"  :model="formData" :rules="formRules"   layout="vertical">
					...
					<a-form-item label="支出账户：" name="accountId">
					...
					<a-form-item label="银行卡号：" name="bankAccount">
						<a-input placeholder="请输入银行卡号" v-model:value="formData.bankAccount"></a-input>
					</a-form-item>

					<a-form-item label="支出日期：" name="payerTime">
...
<script setup name="bizCollectionReceiptForm">
...
const onSubmit = async  () => {
	...
	try {
		...
		await  bizCollectionReceiptApi.batchExpenditure(formDataParam)
		onClose()
		emit('successful')
	}finally {
