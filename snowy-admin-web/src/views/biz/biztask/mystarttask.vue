<template>
		...
		>
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						<a-button :disabled="!isEditRecord(record)" type="link" @click="openEditProcess(record)">
							修改流程
						</a-button>
	...
	<EditAskForLeave @successful="tableRef.refresh()" ref="editAskForLeaveRef" />
</template>
...
<script setup name="historyTask">
	...
	import { ref } from 'vue'
	import EditAskForLeave from './editForm/personnel/editAskForLeave.vue'
	...
	const isEditRecord = (record) => {
		return ['Process_ask_leave'].includes(record.category) && record.endTime == null && record.isEdit
	}
	const openEditProcess = (record) => {
		editAskForLeaveRef.value.onOpen(record)
	}
	...
	const loadData = (parameter) => {
		...
		return bizProcessPage.bizProcessPage(Object.assign(parameter, searchFormParam)).then((data) => {
			return data
		})
	}
	// 重置
