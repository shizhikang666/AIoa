<template>
			...
			<a-tab-pane key="info" tab="基本信息">
				<a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical">
					...
					<a-form-item label="封面图片：" name="coverImage">
						<xn-upload v-model:value="formData.coverImage" :uploadMode="'image'"></xn-upload>
					</a-form-item>
				</a-form>
			</a-tab-pane>
					...
					>
						<template #bodyCell="{ column, text, record, index }">
							...
							<template v-if="column.dataIndex === 'operation'">
								<a-button @click="openEditForm(index)" type="link" size="small">编辑</a-button>
	...
	<edit-single-form @successful="onEdit" ref="editSingleFormRef"></edit-single-form>
</template>

<script setup name="bizProductForm">
	...
	import EditSingleForm from '@/views/biz/bizproduct/editSingleForm.vue'

	const editSingleFormRef = useTemplateRef('editSingleFormRef')
	...
	const openEditForm = (index) => {
		const record = cloneDeep(kitProductForm.value.productList[index])
		editSingleFormRef.value.onOpen(record)
	}

	const onEdit = (record) => {
		const find = kitProductForm.value.productList.find((item) => item.id === record.id)
		if (!find) return

		find.salePrice = record.salePrice

		find.purchasePrice = record.purchasePrice
	}
	...
	const onOpen = async (record) => {
		...
		if (record) {
			...
			try {
				...
				const res = await bizProductApi.bizProductDetail({ id: recordData.id })
				kitProductForm.value.productList = res.productList.map((v) => ({ ...v.product, number: v.number }))
			} finally {
