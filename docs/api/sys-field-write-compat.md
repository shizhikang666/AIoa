...
- `snowy-admin-web/src/views/sys/resource/menu/index.vue`, through the menu row `字段权限` drawer entry
...
- Parent menu delete and parent module delete also cascade to descendant `FIELD` rows and remove direct role-resource relation rows targeting those fields.
- The copied system menu page exposes field maintenance from the row-level `更多` menu for `MENU` rows only. The dynamic route path is `/sys/menu`; the component is `sys/resource/menu/index`.

## Intentional Limits

- No Java source, database schema, Composer files, `.env`, or public config files are changed.
...
Additional browser smoke on 2026-06-11:

- Started MySQL/Redis through the user-provided local runtime bundle, ThinkPHP on `http://127.0.0.1:82`, and Vue on `http://127.0.0.1:83`.
- Logged in with ignored local `.env` credentials and tenant id `1`.
- The current local admin menu data did not include `/sys/menu`, so the smoke temporarily inserted user-resource relations for `/sys/module` and `/sys/menu`, verified the page, then deleted those temporary relation rows.
- Opened `/sys/menu`, expanded a catalog row, clicked a `MENU` row's `更多`, confirmed both `按钮权限` and `字段权限` are visible, clicked `字段权限`, and verified the drawer opened with `GET /api/sys/field/page`.
