...
- `-ReleasePackageBuild` is the fail-closed, read-only artifact mode used by the release builder. It accepts only `-CheckReleasePackagePolicy` plus `-ReleaseRoot`; package-policy issues are fatal, while `.env`, PHP/Composer commands, writable probes, ThinkPHP boot, network, database, and other host/runtime checks are intentionally not run. The package must exclude `.env`, and atomic prepare must still inject and verify a separately approved private environment.
