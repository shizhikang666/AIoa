...
| Current ThinkPHP routes | 385 | From `php think route:list` after user export download route addition |
| Endpoints already covered by route path | 362 | Includes read adapters, auth/system routes, index schedule/message routes, user-center self-service routes, user grants, status switches, reset-password/delete compatibility, organization and position write compatibility, user export download compatibility, and selected low-risk writes |
...
| `biz/dict` | `page`, `tree`, `treeAll`, and `edit` covered; add/delete remain deferred |
...
| `biz/org`, `biz/user`, `biz/position`, `biz/dict` | user import, real Office export rendering, resource/permission grants, dictionary add/delete | `/biz/org/add`, `/biz/org/edit`, `/biz/org/delete`, `/biz/position/add`, `/biz/position/edit`, `/biz/position/delete`, `/biz/user/add`, `/biz/user/edit`, `/biz/user/center/edit`, `/biz/user/delete`, `/biz/user/grantRole`, `/biz/user/disableUser`, `/biz/user/enableUser`, `/biz/user/resetPassword`, `/biz/user/export`, `/biz/user/exportUserInfo`, and `/biz/dict/edit` are covered; user import, real `.xlsx`/`.docx` rendering, and dictionary add/delete remain deferred |
...
| `sys/user`, `sys/userCenter`, and `sys/index` | `import`, real `.xlsx`/`.docx` rendering, encrypted-field migration | Current-user profile/password/workbench/process-config writes, admin-side user add/edit, user delete, user role/resource/permission grant saves, user enable/disable, admin reset password, homepage schedule, message read-state writes, and user export/download blobs are covered |
