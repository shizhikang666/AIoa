...
| Current ThinkPHP routes | 301 | From `php think route:list` after team-project comment add route addition |
| Endpoints already covered by route path | 289 | Includes read adapters, auth/system routes, and selected low-risk writes |
| Missing read/selector/report candidates | 69 | Priority candidates for safe compatibility work |
| Deferred write/side-effect candidates | 187 | Add/edit/delete/audit/import/export/workflow/finance/stock actions |
...
| `biz/teamproject`, `biz/task` | Team project, task, task user, project comment, project comment reply, task comment read slices, and project comment/reply add compatibility |
...
| `biz/bizteamprojectcomment` | `page`, `list`, `detail`, and `add` covered; delete and notification side effects remain deferred |
| `biz/bizteamprojectcommentreply` | `page`, `detail`, and `add` covered; edit/delete remain deferred |
...
| `biz/teamproject comments` | comment delete, reply edit/delete, notification push, data-change events | Comment add and reply add are covered with member guard; existing-history mutations and push side effects remain deferred |
