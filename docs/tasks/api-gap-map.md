...
| Current ThinkPHP routes | 312 | From `php think route:list` after team-project task category maintenance route addition |
| Endpoints already covered by route path | 300 | Includes read adapters, auth/system routes, and selected low-risk writes |
| Missing read/selector/report candidates | 69 | Priority candidates for safe compatibility work |
| Deferred write/side-effect candidates | 176 | Add/edit/delete/audit/import/export/workflow/finance/stock actions |
...
| `biz/teamproject`, `biz/task` | Team project, task, task category, task user, project comment, project comment reply, task comment read/add/edit/delete slices, task category maintenance, task assignee sync, and project comment/reply base write compatibility |
...
| `biz/teamproject comments and tasks` | notification push, data-change events, full task mutations, task drag/move flows | Comment/reply base writes, task comment add/edit/delete for `COMMENT` rows, task category add/edit/sort/delete, and task assignee sync are covered with member/resource-permission guards; push and data-change side effects remain deferred |
