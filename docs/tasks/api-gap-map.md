...
| Current ThinkPHP routes | 308 | From `php think route:list` after team-project task comment maintenance route addition |
| Endpoints already covered by route path | 296 | Includes read adapters, auth/system routes, and selected low-risk writes |
| Missing read/selector/report candidates | 69 | Priority candidates for safe compatibility work |
| Deferred write/side-effect candidates | 180 | Add/edit/delete/audit/import/export/workflow/finance/stock actions |
...
| `biz/teamproject`, `biz/task` | Team project, task, task user, project comment, project comment reply, task comment read/add/edit/delete slices, task assignee sync, and project comment/reply base write compatibility |
...
| `biz/teamproject comments and tasks` | notification push, data-change events, full task/category mutations | Comment/reply base writes, task comment add/edit/delete for `COMMENT` rows, and task assignee sync are covered with member/resource-permission guards; push and data-change side effects remain deferred |
