...
Last updated: 2026-06-06 11:21 +08:00
...
| Frontend endpoints already routed | 324 | Matched against current ThinkPHP route paths |
...
| Latest frontend-slice commit | See latest Git log | Current dashboard records index message all-mark-read compatibility |
...
| User / Org / Position | 76% | Yellow | Read-only org tree, position, user directory, user-center selectors, business-side directory aliases, camelCase display aliases for org/user/position pages, system user grant echo reads, current-user profile/password/avatar/signature/workbench/process-config writes, user/index message detail mark-read, and homepage all-message mark-read | User CRUD, grant writes, admin reset password, import/export, encrypted profile fields, message send/delete |
...
| User directory | Org/user/position selectors, read-only directory, current-user self-service profile/password/workbench/process-config writes, message detail mark-read, and homepage all-message mark-read | Admin-side CRUD and grants deferred |
