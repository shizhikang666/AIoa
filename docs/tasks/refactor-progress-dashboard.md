...
Last updated: 2026-06-06 10:55 +08:00
...
| User / Org / Position | 74% | Yellow | Read-only org tree, position, user directory, user-center selectors, business-side directory aliases, camelCase display aliases for org/user/position pages, system user grant echo reads, current-user profile/password/avatar/signature/workbench/process-config writes | User CRUD, grant writes, admin reset password, import/export, encrypted profile fields |
...
| User directory | Org/user/position selectors, read-only directory, and current-user self-service profile/password/workbench/process-config writes | Admin-side CRUD and grants deferred |
...
| Sys user grant echo reads and user-center writes | `/sys/user/list/detail`, `/ownRole`, `/ownResource`, and `/ownPermission` routed for copied user grant dialogs; `/sys/userCenter/updatePassword`, `/updateAvatar`, `/updateSignature`, `/updateUserInfo`, `/updateUserWorkbench`, and `/process/config/edit` routed for current-user self-service writes | Grant role/resource/permission writes, user CRUD, enable/disable, admin reset password, import/export deferred |
