...
Last updated: 2026-06-05 12:20 +08:00
...
| Registered route entries | 280 | Most are protected read-only compatibility routes |
| API compatibility docs | 64 | Stored under `docs/api` |
...
| Frontend endpoints already routed | 268 | Matched against current ThinkPHP route paths |
| Frontend missing read/selector candidates | 70 | Priority candidates for safe compatibility slices |
...
| Latest frontend-slice commit | See latest Git log | Current dashboard records gen basic metadata read compatibility |
...
| Frontend adaptation | 68% | Yellow | Original Vue project copied into target repo; request prefix, Bearer token, upload/SSE token headers, local SM2 fallback, double-prefix fix, menu leaf handling adapted, API gap map generated, org/user display aliases added, sys-user grant echo reads added, sys-field reads added, gen basic metadata reads added, copy-task CC record reads added, sale-project draft detail read added, annual-leave balance page/detail read added, sale-project invoice item page read added, sale-project field change log reads added, team-project task user reads added, team-project comment/reply reads added, dev monitor network read added, sale-project rating detail read added, minimal SSE route added, short-lived SSE client fallback added; browser smoke reaches `/sys/org`, `/sys/user`, `/biz/bizdatareport/summaryStatistics`, `/biz/saleproject` with visible pagination, sale-project detail read tabs, and cost tab zero-revenue display is guarded | Broken API method cleanup, remaining read-only business routes |
