...
Last updated: 2026-06-05 16:12 +08:00
...
| Registered route entries | 306 | Most are protected compatibility routes; customer, sale-project follow-up, product-info, field-change-log, history-Excel, project-rate, CC-record delete, team-project comment/reply base writes, team-project task assignee sync, and task comment add are now open |
...
| Frontend endpoints already routed | 294 | Matched against current ThinkPHP route paths |
...
| Frontend deferred write/side-effect candidates | 182 | Do not implement without module-specific write plans |
...
| Latest frontend-slice commit | See latest Git log | Current dashboard records team-project task comment add compatibility |
...
| Business reads/writes | Product, supplier, settlement account, payment record, expenditure record, collection receipt, debit note, file relation, team project, task/comments, warehouses, inventory, delivery, purchase order, return order, sale project reads; customer follow-up, sale-project follow-up, product-info, field-change-log, history-Excel, project-rate, CC-record delete, and team-project comment/reply base writes | Most business writes deferred |
...
| Customer reads and follow-up writes | `/biz/customer/page`, `/detail`, `/detail/list`, `/biz/customerfollowup/page`, `/detail`, `/add`, `/edit`, and `/delete` routed with customer owner/org/file, follow-up creator display fields, data-scope write guarding, and logical delete | Customer writes, owner reassignment, attachment upload/storage cleanup, notifications, and SM4 plaintext search deferred |
...
| Team project task comment add | `/biz/bizteamprojecttaskcomment/add` routed for copied task detail comment submission, deriving the team project from the task and preserving `EXT_JSON.file` attachment metadata | Task comment edit/delete, notifications, and data-change events deferred |
