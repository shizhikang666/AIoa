...
| Business read-only APIs | 79% | Yellow | Product, supplier, warehouse, inventory, delivery, purchase, settlement, payment, expenditure, collection, debit, file relation, team project, return order, sale project, customer, customer follow-up, sale-project invoicing, invoice, reissue, project-rate, sale-project product info, biz-datareport sale-project summaries/unpaid/details/settlement/sale-profit/summary-statistics reads, leave-application reads, settlement-account-payment reads, payroll reads, workflow process/task reads | Remaining detail consumers |
...
3. Start the next safe read-only business slice after a quick frontend/backend smoke of the summary statistics page.
4. Browser-smoke `/sys/org`, `/sys/user`, and customer/detail/report pages after each backend or frontend compatibility slice.
