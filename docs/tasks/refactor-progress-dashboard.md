...
| Registered route entries | 244 | Most are protected read-only compatibility routes |
| API compatibility docs | 52 | Stored under `docs/api` |
...
| Frontend endpoints already routed | 238 | Matched against current ThinkPHP route paths |
| Frontend missing read/selector candidates | 100 | Priority candidates for safe compatibility slices |
...
| Business read-only APIs | 76% | Yellow | Product, supplier, warehouse, inventory, delivery, purchase, settlement, payment, expenditure, collection, debit, file relation, team project, return order, sale project, customer, customer follow-up, sale-project invoicing, invoice, reissue, project-rate, sale-project product info, biz-datareport sale-project summaries/unpaid/details, leave-application reads, settlement-account-payment reads, payroll reads, workflow process/task reads | Remaining reports and detail consumers |
...
3. Add remaining business report read slices: sale profit, settlement income/expenses, and summary statistics.
