...
Last updated: 2026-06-06 14:10 +08:00

Agent: merge-agent / main control agent

## Current Estimate

Overall production-ready completion: **64%**
...
| Frontend endpoints already routed | 327 | Matched against current ThinkPHP route paths |
...
| Frontend deferred write/side-effect candidates | 152 | Do not implement without module-specific write plans |
...
| Latest frontend-slice commit | See latest Git log | Current dashboard records user enable/disable compatibility |
...
| User / Org / Position | 84% | Yellow | Read-only org tree, position, user directory, user-center selectors, business-side directory aliases, camelCase display aliases for org/user/position pages, system user grant echo reads, user role/resource/permission grant saves, user enable/disable, current-user profile/password/avatar/signature/workbench/process-config writes, user/index message detail mark-read, homepage all-message mark-read, and homepage schedule add/delete | User CRUD, admin reset password, import/export, and encrypted profile fields |
...
| User directory | Org/user/position selectors, read-only directory, role/resource/permission grant saves, user enable/disable, current-user self-service profile/password/workbench/process-config writes, message detail mark-read, homepage all-message mark-read, and homepage schedule add/delete | Admin-side CRUD deferred |
...
| Sys user grants/status and user-center/index writes | `/sys/user/list/detail`, `/ownRole`, `/ownResource`, `/ownPermission`, `/sys/user/grantRole`, `/biz/user/grantRole`, `/sys/user/grantResource`, `/sys/user/grantPermission`, `/sys/user/disableUser`, `/sys/user/enableUser`, `/biz/user/disableUser`, and `/biz/user/enableUser` routed for copied user grant/status dialogs; `/sys/userCenter/updatePassword`, `/updateAvatar`, `/updateSignature`, `/updateUserInfo`, `/updateUserWorkbench`, `/process/config/edit`, detail-time message mark-read, `/sys/index/message/allMessageMarkRead`, `/sys/index/schedule/add`, and `/sys/index/schedule/deleteSchedule` routed for current-user self-service flows | User CRUD, admin reset password, and import/export deferred |
