...
class MenuController extends BaseSysController
{
    ...
    {
        return $this->guard(fn () => $this->resourceService->menuAdd(
    ...
    {
        return $this->guard(fn () => $this->resourceService->menuEdit(
            $this->bodyInput($request),
            $request->middleware('auth_payload', [])
        ));
    }

    public function changeModule(Request $request): Response
    {
        return $this->guard(fn () => $this->resourceService->menuChangeModule(
            $this->bodyInput($request),
            $request->middleware('auth_payload', [])
        ));
    }
    ...
    public function delete(Request $request): Response
    {
        return $this->guard(function () use ($request): array {
            ...
            return $this->resourceService->menuDelete(
    ...
    private function deleteIds(array $input): array
    {
        ...
        foreach (['idList', 'ids', 'id', 'menuIds'] as $key) {
