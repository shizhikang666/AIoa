...
class SaleProjectFollowUpController extends BaseSysController
{
    ...
    {
        return $this->guard(fn () => $this->saleProjectFollowUpService->add($this->body($request), $this->authPayload($request)));
    }

    public function edit(Request $request): Response
    {
        return $this->guard(fn () => $this->saleProjectFollowUpService->edit($this->body($request), $this->authPayload($request)));
    }
    ...
    public function delete(Request $request): Response
    {
        return $this->guard(function () use ($request): array {
            ...
            return $this->saleProjectFollowUpService->delete($this->deleteIds($request, $input), $this->authPayload($request));
        });
    }
