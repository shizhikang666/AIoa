...
class SalesProjectFieldChangeLogController extends BaseSysController
{
    ...
    {
        return $this->guard(fn () => $this->changeLogService->add($this->body($request), $this->authPayload($request)));
    }

    public function edit(Request $request): Response
    {
        return $this->guard(fn () => $this->changeLogService->edit($this->body($request), $this->authPayload($request)));
    }
    ...
    public function delete(Request $request): Response
    {
        return $this->guard(function () use ($request): array {
            ...
            return $this->changeLogService->delete($this->deleteIds($request, $input), $this->authPayload($request));
        });
    }
