...
class BizHistoryExcelController extends BaseSysController
{
    ...
    {
        return $this->guard(fn () => $this->historyExcelService->add($this->body($request), $this->authPayload($request)));
    }

    public function edit(Request $request): Response
    {
        return $this->guard(fn () => $this->historyExcelService->edit($this->body($request), $this->authPayload($request)));
    }
    ...
    public function delete(Request $request): Response
    {
        return $this->guard(function () use ($request): array {
            ...
            return $this->historyExcelService->delete($this->deleteIds($request, $input), $this->authPayload($request));
        });
    }
