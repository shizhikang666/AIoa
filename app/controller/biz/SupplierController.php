...
class SupplierController extends BaseSysController
{
    ...
    {
        return $this->guard(fn () => $this->supplierService->add($this->body($request), $this->authPayload($request)));
    }

    public function edit(Request $request): Response
    {
        return $this->guard(fn () => $this->supplierService->edit($this->body($request), $this->authPayload($request)));
    }
    ...
    public function delete(Request $request): Response
    {
        return $this->guard(function () use ($request): array {
            ...
            return $this->supplierService->delete($this->deleteIds($request, $input), $this->authPayload($request));
        });
    }
