...
class CustomerFollowUpController extends BaseSysController
{
    ...
    {
        return $this->guard(fn () => $this->customerFollowUpService->add($this->body($request), $this->authPayload($request)));
    }

    public function edit(Request $request): Response
    {
        return $this->guard(fn () => $this->customerFollowUpService->edit($this->body($request), $this->authPayload($request)));
    }
    ...
    public function delete(Request $request): Response
    {
        return $this->guard(function () use ($request): array {
            ...
            return $this->customerFollowUpService->delete($this->deleteIds($request, $input), $this->authPayload($request));
        });
    }
